...
data_load <- function(files){
  ...
  for(i in 1:length(files)){
    ...
    dat_list[[i]] <- wrangling(fread(files[i])) %>% mutate(test = i-1)
    
    }
  
  dat <- rbindlist(data_list) %>% filter(DO != '<EOF>')
...
tPD_plot <- function(dat){
  ...
  ggsave(paste0('C:\\Users\\mano.hong\\Desktop\\lscl\\', mylotname, '\\', mylotname, '_tPDYLD.png'))
...
NB_plot <- function(dat){
  ...
  ggsave(paste0('C:\\Users\\mano.hong\\Desktop\\lscl\\', mylotname, '\\', mylotname, '_NB.png'))
...
byRUN_plot <- function(dat){
  ...
  ggsave(paste0('C:\\Users\\mano.hong\\Desktop\\lscl\\', mylotname, '\\', mylotname, '_RUN.png'))
...
MAP_plot <- function(dat){
  ...
  ggsave(paste0('C:\\Users\\mano.hong\\Desktop\\lscl\\', mylotname, '\\', mylotname, '_WFMAP.png'))
...
MAPRUN_plot <- function(dat){
  ...
  ggsave(paste0('C:\\Users\\mano.hong\\Desktop\\lscl\\', mylotname, '\\', mylotname, '_WFMAPRUN.png'))
...
MAPtPD_plot <- function(dat){
  ...
  ggsave(paste0('C:\\Users\\mano.hong\\Desktop\\lscl\\', mylotname, '\\', mylotname, '_WFMAP(tPD).png'))
...
MAPtPDRUN_plot <- function(dat){
  ...
  ggsave(paste0('C:\\Users\\mano.hong\\Desktop\\lscl\\', mylotname, '\\', mylotname, '_WFMAPRUN(tPD).png'))
...
data_load <- function(test_n){
  ...
  if(test_n == 1){
    ...
    data_list$prime <- wrangling(read.csv(paste0(mylotname, '_0.csv'), header = F)) %>% 
      filter(!SG %in% data_list$retest1$SG)
    
  }else if(test_n == 3){
    
    data_list$retest2 <- wrangling(read.csv(paste0(mylotname, '_2.csv'), header = F)) 
    data_list$retest1 <- wrangling(read.csv(paste0(mylotname, '_1.csv'), header = F)) %>% 
      filter(!SG %in% data_list$retest2$SG)
    data_list$prime <- wrangling(read.csv(paste0(mylotname, '_0.csv'), header = F)) %>% 
      filter(!SG %in% data_list$retest1$SG &
               !SG %in% data_list$retest2$SG)
    
  }else if(test_n == 4){
    
    data_list$retest3 <- wrangling(read.csv(paste0(mylotname, '_3.csv'), header = F)) 
    data_list$retest2 <- wrangling(read.csv(paste0(mylotname, '_2.csv'), header = F)) %>% 
      filter(!SG %in% data_list$retest3$SG)
    data_list$retest1 <- wrangling(read.csv(paste0(mylotname, '_1.csv'), header = F)) %>% 
      filter(!SG %in% data_list$retest3$SG & 
               !SG %in% data_list$retest2$SG)
    data_list$prime <- wrangling(read.csv(paste0(mylotname, '_0.csv'), header = F)) %>% 
